...
async fn pdf_download_test_server(body: &'static [u8]) -> (String, tokio::task::JoinHandle<usize>) {
    use tokio::io::{AsyncReadExt, AsyncWriteExt};
...
fn test_session_for_pdf_downloads(
    ...
    CdpBrowserSession {
        ...
        interaction_highlight: InteractionHighlightConfig::from_profile(&BrowserProfile::default()),
...
async fn cdp_command_header_test_server(
    ...
    let handle = tokio::spawn(async move {
        ...
        let handshake_headers = Arc::new(std::sync::Mutex::new(BTreeMap::<String, String>::new()));
...
fn cdp_command_test_response(id: u64, method: &str, grant_error: Option<&'static str>) -> Value {
...
fn default_profile_uses_upstream_ignore_default_args_shape() {
    ...
    let ignored_all: BrowserProfile =
        serde_json::from_value(json!({ "ignore_default_args": true })).expect("ignore all profile");
    assert_eq!(
...
async fn direct_connect_records_permission_grant_failures_without_failing() {
    let (endpoint, command_log) = cdp_command_test_server(Some("permission grant denied"), 7).await;
    let session = CdpBrowserSession::connect(endpoint)
        .await
...
async fn cloud_browser_client_contextualizes_non_success_errors() {
    ...
    assert!(
        error
            .to_string()
            .contains("Failed to stop cloud browser: HTTP 503 Service Unavailable - stop failed")
    );
...
fn iframe_target_limits_honor_profile_controls() {
    ...
    assert!(iframe_target_infos_from_targets(&targets, "root", &frame_infos, disabled).is_empty());
    ...
    assert!(
        iframe_target_infos_from_targets(&targets, "root", &frame_infos, zero_iframes).is_empty()
    );
}
...
fn browser_lifecycle_events_cover_remaining_upstream_shapes() {
    let events = vec![
        ...
        BrowserLifecycleEvent::download_progress("download-guid", 1024, Some(4096), "inProgress"),
...
fn lifecycle_watchdog_maps_cdp_crash_and_download_events() {
    ...
    let reconnected = lifecycle_event_for_websocket_reconnected(&cdp_websocket_reconnected_event(
        "http://127.0.0.1:9222",
        2,
...
fn parses_devtools_active_port_endpoint() {
    let endpoint =
        DevToolsEndpoint::from_active_port_file("127.0.0.1", "38119\n/devtools/browser/abc123\n")
...
fn dropdown_scripts_can_run_as_cached_element_functions() {
    ...
    let select_body = select_dropdown_option_body_js("Enterprise").expect("select dropdown body");
...
fn interactive_snapshot_filters_occluded_elements() {
    ...
    assert!(
        INTERACTIVE_ELEMENTS_JS
            .contains("(!paintOrderFiltering || isNativeMediaControl || isTopmostAtCenter(el))")
    );
...
fn interactive_snapshot_script_carries_paint_order_filtering_control() {
    ...
    assert!(
        enabled.contains("(!paintOrderFiltering || isNativeMediaControl || isTopmostAtCenter(el))")
    );
...
async fn cdp_session_indexes_javascript_listener_elements() {
    ...
    let listener = initial_state
        ...
        .selector_map
        .values()
        .find(|element| element.attributes.get("id").map(String::as_str) == Some("plain-listener"))
        .unwrap_or_else(|| {
            panic!(
