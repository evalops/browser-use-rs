...
use browser_use_cdp::{
    BrowserError, BrowserLifecycleEvent, BrowserLifecycleEventKind, BrowserSession, FoundElement,
    Pdf, Screenshot, browser_lifecycle_adapter_events,
};
...
fn mixed_interactive_state_can_render_opt_in_ax_description() {
    let rendered =
        mixed_interactive_state().llm_representation_with_attributes(&["description".to_owned()]);
...
fn mixed_interactive_state_preserves_quiet_ax_clickability_metadata() {
    ...
    assert!(
        state
            ...
            .contains("[5] <div focusable=true editable=true settable=true> Draft note Draft body")
    );
}

#[test]
fn frame_shadow_state_matches_golden_fixture() {
    let expected: Value = serde_json::from_str(include_str!("../fixtures/frame_shadow_state.json"))
        .expect("golden fixture");
...
fn browser_lifecycle_exception_events_match_golden_fixture() {
    let actual = serde_json::to_value(vec![
        ...
        BrowserLifecycleEvent {
            ...
            message: "JavaScript confirm dialog on https://example.test was accepted: Continue?"
                .to_owned(),
        },
        BrowserLifecycleEvent {
            ...
            message: "Download download-guid progress: completed (4096 bytes received)".to_owned(),
        },
        BrowserLifecycleEvent {
            ...
            message: "Storage state saved to /tmp/storage.json (4 cookies, 2 origins)".to_owned(),
        },
    ])
    .expect("serialize lifecycle events");
...
impl ActionExecutor for FixtureExecutor {
    async fn execute(&mut self, action: &BrowserAction) -> ActionResult {
        match action {
            ...
            other => ActionResult::error(format!("unexpected fixture action: {}", other.name())),
        }
    }
}
...
impl BrowserSession for ReplayRecaptureFixtureSession {
    async fn state(&self, _include_screenshot: bool) -> Result<BrowserStateSummary, BrowserError> {
        self.states
            .lock()
            .expect("states lock")
            .pop_front()
            .ok_or_else(|| BrowserError::StateUnavailable("fixture states exhausted".to_owned()))
    }
    ...
    async fn select_dropdown_option(&self, _index: u32, _text: &str) -> Result<(), BrowserError> {
