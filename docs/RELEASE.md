...
- Local Chrome/Chromium launch and CDP attach, including upstream-style
  ...
  Chrome flags for CI and Docker-style launches.
...
- Managed `FileSystem` state with a `browseruse_agent_data` sandbox directory,
  ...
  Restored agents can continue from serialized `FileSystemState` with
...
- Agent runs with schema-guided provider output, upstream-style initial actions,
  ...
  detection, previous result context,
  ...
  requests while clearing pause/stop control state. Host runtimes can also
  register external-status interruption callbacks that abort before model or
  browser work without setting durable stopped state.
