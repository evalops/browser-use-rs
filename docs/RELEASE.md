...
- Browser state with URL, title, tabs plus browser-use-style short tab ids,
  ...
  click/pointer listener-backed controls, pagination affordance detection,
...
- Agent runs with schema-guided provider output, max actions per step, max
  ...
  clickable-element text limits, upstream-style prompt context/error
  truncation, typed upstream-style last-result completion helpers, upstream-compatible
