...
- Agent runs with schema-guided provider output, upstream-style initial actions,
  ...
  final `done` responses after repeated failures, upstream-style final-step
  done-only guard when `max_steps` is reached, upstream-style 75% step-budget
  warning before finalization, normalized repeated-action loop detection,
