...
- Browser Use Cloud creation and stop-session request/response contracts,
  including conversion of `cdpUrl` responses into CDP endpoints,
  ...
  requests, auth errors, missing-session errors, and current-session cleanup on
  successful stop or 404.
