...
- DOM indexing is compact and accessibility-aware, including same-origin iframe
  ...
  duplicate long-attribute pruning and JavaScript click/pointer listener-backed
  control detection when Chrome exposes command-line inspection APIs. Full
  browser-use DOM/AX snapshot parity is still tracked separately.
